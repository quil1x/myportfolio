...
class _ThemeAnimatedFaunaState extends State<ThemeAnimatedFauna> with SingleTickerProviderStateMixin {
  List<_FaunaParticle> _particles = []; 
  ...
  void _generateFauna() {
    if (!mounted || _areaSize == null || _areaSize!.height <= 0 || _areaSize!.width <= 0) {
      ...
      // Якщо _areaSize не встановлено, ми не можемо генерувати частинки.
      // Переконуємося, що _particles порожній, щоб build метод повернув SizedBox.
    ...
    _particles.clear(); // Завжди очищуємо перед новою генерацією
    String imagePath;
    Size faunaSize;
    int count;
    ...
    if (faunaSize.height > _areaSize!.height || maxCreatureVerticalSpace < 1.0 && faunaSize.height > 0) { 
        ...
        if(mounted) setState(() {}); // Переконатися, що UI оновлюється, якщо _particles змінився
        return; 
    }
  ...
  Widget build(BuildContext context) {
    ...
    // Якщо _areaSize ще не ініціалізовано, або немає частинок, показуємо порожній контейнер
    // Це важливо, щоб уникнути помилок, поки розміри не визначені
        ...
        children: _particles.map((particle) {
          Widget imageWidget = Image.asset(
            ...
            errorBuilder: (context, error, stackTrace) {
              ...
              return Container( // ⬅️ Змінено на синій для кращої видимості помилки
                width: particle.size.width, 
                height: particle.size.height, 
